...
function void top_env::build_phase(uvm_phase phase);
  ...
  if (!uvm_config_db#(virtual {{ name }}_if)::get(this, "m_{{ name }}_agent", "vif", m_{{ name }}_config.vif)) begin
    `uvm_fatal(get_name(), "Could not retrieve m_{{ name }}_config from config db")
  end
